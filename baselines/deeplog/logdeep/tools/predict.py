...
class Predicter():
    ...
    def predict_unsupervised(self):
        ...
        test_normal_loader, test_normal_length = generate(self.data_dir
                                                          + 'test_normal.csv')
            ...
            for line in tqdm(test_normal_loader.keys()):
                for i in range(len(line) - self.window_size):
                    ...
                    if label not in predicted:
                        ...
                        sequence_fp.append(line)
                        break
        with torch.no_grad():
            for line in tqdm(test_abnormal_loader.keys()):
                sequence_abnormal.append(line)
                for i in range(len(line) - self.window_size):
                    seq0 = line[i:i + self.window_size]
                    label = line[i + self.window_size]
                    seq1 = [0] * self.num_classes
        ...
        F1 = 2 * P * R / (P + R)
        print(
            'TP: {}, false positive (FP): {}, false negative (FN): {}, Precision: {:.3f}%, Recall: {:.3f}%, F1-measure: {:.3f}%'
        ...
        df_p = pd.DataFrame(performance, index=['TP',
                                                ...
                                                'R',
                                                'F1'])
        df_p.to_csv(self.save_dir + 'performance.csv')
        print('Finished Predicting')
        elapsed_time = time.time() - start_time
        print('elapsed_time: {}'.format(elapsed_time))

        sequence_fn = list(set(sequence_abnormal) - set(sequence_tp))
        self.save_FalseResults(sequence_fp, 'fp.csv')
        self.save_FalseResults(sequence_fn, 'fn.csv')

    def save_FalseResults(self, list_false, save_file):
        with open(self.save_dir + save_file, 'w') as f:
            for line in list_false:
                f.write(' '.join([str(event) for event in line]) + '\n')
